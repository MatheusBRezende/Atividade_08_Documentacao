...
|-------------|------|-----------|------------|
| GET | /api/autores | Listar todos os autores. | Nenhum |
| GET | /api/autores/{id} | Buscar um autor específico por ID. | id (path) |
| POST | /api/autores | Cadastrar um novo autor. | Corpo da Requisição (Autor) |
| PUT/PATCH | /api/autores/{id} | Atualizar dados do autor. | id (path), Corpo da Requisição |
...
- Autor, Livro, Emprestimo 
- Quais campos são obrigatórios (`[Required]`)?
-LivroId
- Quais validações existem (`[StringLength]`, `[Range]`)?
- Restrições de negócio (ex: [Required] para campos obrigatórios; [StringLength] para tamanho máximo/mínimo; [Range] para valores numéricos).
- Qual a relação entre as entidades? (Autor → Livro → Empréstimo)
-Um Autor tem muitos Livros; um Livro está associado a um Empréstimo
...
- Sim os endpoints aparecem e estão corretamente agrupados
...
- A documentação atual cobre a sintaxe básica dos recursos (CRUD), mas falta profundidade na documentação de casos de uso. É essencial adicionar documentação detalhada sobre erros de negócio (como o livro já emprestado, que deve ser um 409 Conflict), incluir parâmetros de consulta (para filtrar buscas em lista) e formalizar o retorno de listas paginadas, garantindo que o consumidor final tenha um guia completo para interagir com o sistema.
